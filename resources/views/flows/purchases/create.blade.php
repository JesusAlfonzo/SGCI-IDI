            ...
            <form action="{{ route('flows.purchases.store') }}" method="POST" id="purchaseForm">
                ...
                <div class="row mb-4">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        @error('supplier_id') <span class="invalid-feedback"><strong>{{ $message }}</strong></span> @enderror
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        @error('purchase_date') <span class="invalid-feedback"><strong>{{ $message }}</strong></span> @enderror
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        @error('invoice_number') <span class="invalid-feedback"><strong>{{ $message }}</strong></span> @enderror
                    </div>
                </div>
                    ...
                    <table class="table table-striped" id="purchaseDetailsTable">
                            ...
                            @if (old('details'))
                                @foreach (old('details') as $index => $detail)
                                    <tr>
                                        <td>
                                            <select name="details[{{ $index }}][product_id]" class="form-control product-select" required>
                                                ...
                                                @foreach ($products as $product)
                                                    <option value="{{ $product->id }}" data-unit="{{ $product->unit->name ?? 'N/A' }}" {{ old("details.$index.product_id") == $product->id ? 'selected' : '' }}>
                                        ...
                                        {{-- Importante: min="0.00" --}}
                                        <td><input type="number" name="details[{{ $index }}][unit_cost]" class="form-control unit-cost" step="0.0001" min="0.00" required value="{{ old("details.$index.unit_cost", 0.00) }}"></td>
                                        <td><input type="number" name="details[{{ $index }}][quantity]" class="form-control quantity" min="1" required value="{{ old("details.$index.quantity", 1) }}"></td>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td>
                                    <input type="text" id="totalDisplay" class="form-control" value="{{ old('total_amount', 0.00) }}" readonly>
                                    {{-- Campo oculto para enviar el total al controlador --}}
                                    <input type="hidden" name="total_amount" id="totalAmountInput" value="{{ old('total_amount', 0.00) }}">
                                </td>
                                <td></td>
                            </tr>
                        </tfoot>
                ...
                <div class="d-flex justify-content-between mt-3">
                    <button type="button" class="btn btn-info" id="addRowButton"><i class="fas fa-plus"></i> Añadir Ítem</button>
                    <div>
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            var productOptions = '';
            @foreach ($products as $product)
                productOptions += `<option value="{{ $product->id }}" data-unit="{{ $product->unit->name ?? 'N/A' }}">{{ $product->name }} ({{ $product->unit->name ?? 'N/A' }})</option>`;
            @endforeach
